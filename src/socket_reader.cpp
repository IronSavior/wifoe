...
connection socket_reader::watch_socket( const sys::socket_handle sock, event_signal::slot_type slot ) {
  ...
  if( sig_pair == signals.end() ) {
    sig_pair = signals.insert( sig_pair, std::make_pair(sock, std::shared_ptr<event_signal>(new event_signal))  );
  }
...
void socket_reader::loop() {
  ...
  for(;;) {
    ...
    auto read_socks = sys::select_for_read(socket_list());
    for( auto sock : read_socks ) {
      auto buf = sys::read_sock(sock);
      signals[sock]->operator()(buf);
    }
  }
}
